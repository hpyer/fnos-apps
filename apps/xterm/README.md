...
- xterm.js 原生支持 ANSI/256 色；终端默认启用常见的目录、链接、可执行文件颜色，以及 `ls`、`ll`、`la`、`l` 别名和彩色提示符。
- 通过终端内置的 `su` 切换到其他用户后，应用默认配色仍会继续生效；不同用户的 HOME、权限和身份保持由系统控制。应用不加载外部可写的 Bash 配置文件。
- 断开即回收会话；异常断网由心跳检测，通常在 30 秒内回收。没有会话保活或自动重连。
- HOME 为 `${TRIM_PKGVAR}/home`；Bash 不加载飞牛用户的 profile/bashrc，仅加载应用内置配置。
